"""Hidden Markov Model utilities and the Viterbi decoder."""

import numpy as np

# default HMM describing the digit sequences
WORDS = {
...
def limLog(x):
    """Log with floor to avoid numerical issues."""
    MINLOG = 1e-100
    return np.log(np.maximum(x, MINLOG))


def column(ary, col):
    """Return column ``col`` from a 2‑D list or array."""
    colAry = []
    for row in ary:
...
def viterbi(logLike, logPi, logA):
    """Classic Viterbi dynamic programming algorithm."""

    viterbiMatrix = np.zeros((len(logLike), len(logPi)))
    ...
    for t in range(1, seqLen):
        for j in range(number_of_states):
            ...
            pathMatrix[t][j] = np.argmax(np.array(viterbiMatrix[t - 1]) + np.array(column(logA, j)))
...
class HMM:
    """Simple left‑to‑right HMM for isolated digit recognition."""
    ...
    def isIN(self, seq, idx):
        result = 1
        try:
            hypothesis = seq.index(idx)
        except ValueError:
            result = 0
        return result

    def getTranscription(self, ssq):
        """Convert a state sequence into the corresponding word sequence."""
