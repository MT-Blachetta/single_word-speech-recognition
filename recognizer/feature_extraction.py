...
def make_frames(audio_data, sampling_rate, window_size, hop_size):
    ...
    hamming_window = np.hamming(new_window_size)    # Calculating the hamming frame for the specific frame length
    pad_width = new_window_size #-(len(audio_data) % new_window_size)
    padded_audio_data = np.pad(np.array(audio_data), (0, pad_width), 'constant', constant_values=0)
    j = 0   # Index for the sampled audio signal

    for i in range(number_of_frames):       # Looping through each frame and assigning the sampled values including overlap and multiplying with hamming window
        frames[i * new_window_size: (i * new_window_size) + new_window_size ] = np.multiply(padded_audio_data[j: j + new_window_size], hamming_window) 
        j += new_hop_size
...
def get_mel_filters(sampling_rate,freq_idx,n_filters,f_min=0,f_max=8000):
    ...
    mel_max = tools.hz_to_mel(8000);
    test_hz = tools.mel_to_hz(mel_max);
    ...
    for i in range(n_filters+2):
        stuezstelle[i] = int(round(tools.mel_to_hz(i*melInterval)/FreqInterval)); # for the index calculation
