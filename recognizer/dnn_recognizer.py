"""Functions related to the Deep Neural Network acoustic model."""
...
def wav_to_posteriors(model, audio_file, parameters):
    """Run feature extraction and forward pass through the DNN."""
...
def generator(x_dirs, datadir, hidmm, parameters, epoch):
    """Yield training batches for ``model.fit_generator``.

    Training data is read on the fly to avoid loading the full data set into
    memory. For each file the corresponding feature matrix and target state
...
def train_model(datadir, hidmm, model_dir, parameters, epochs=3):
    """Train a DNN using the supplied data set and parameters."""
...
def dnn_model(input_shape, output_shape):
    """Create a simple feed-forward DNN for acoustic modelling."""
    model = Sequential()
